...
class DeploymentManager:
    ...
    def check_ipfs_connection_works(self) -> None:
        ...
        for i in range(3):
            try:
                requests.get(constants.IPFS_CHECK_URL)
                return
            except OSError:
                self.logger.exception(
                    f"failed to connect to ipfs to test connection. OSError, critical!"
                )
                raise
            except:
                self.logger.exception(
                    f"failed to connect to ipfs to test connection. do another try"
                )
                time.sleep(i * 5)
        self.logger.error(
            f"failed to connect to ipfs to test connection. no attempts left. raise error"
        )
        raise RuntimeError(
