...
class StakingHandler:
    ...
    def _get_staking_token_contract(self: "StakingHandler", program_id: str) -> Any:
        ...
        assert (  # nosec
            staking_token_instance_address is not None
        ), f"Staking program {program_id} not found!"
    ...
    def get_staking_contract_metadata(self, program_id: str) -> Dict[str, Any]:
        """Get staking contract metadata."""
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    max_services = (
                        staking_token_contract.functions.maxNumServices().call()
                    )
                    current_services = (
                        staking_token_contract.functions.getServiceIds().call()
                    )
    ...
    def get_staking_env_variables(
        self: "StakingHandler", program_id: str
    ) -> StakingVariables:
        """Get staking environment variables."""
        if program_id == NO_STAKING_PROGRAM_ID:
        ...
        try:
            agent_id = int(staking_token_contract.functions.agentIds(0).call())
        except ContractLogicError:
            agent_id = self.default_agent_id
