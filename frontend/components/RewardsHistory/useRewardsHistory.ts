...
export const useRewardsHistory = () => {
  ...
  const { data, isError, isLoading, isFetching, refetch } = useQuery({
    ...
    select: (data) => {
      ...
      // const [betaRewards, beta2Rewards] = Object.values(allRewards);
      const betaRewards = allRewards[betaAddress.toLowerCase()];
      const beta2Rewards = allRewards[beta2Address.toLowerCase()];

      // empty array
      // const betaRewards = [] as RewardHistoryResponse[];
      // const beta2Rewards = [] as RewardHistoryResponse[];

      // const beta2Rewards = (data as { allRewards: RewardHistoryResponse[] })
      //   .allRewards;
