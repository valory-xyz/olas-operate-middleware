...
class TestGetCurrencySmallestUnit:
    """Tests for get_currency_smallest_unit."""
    ...
    def test_unknown_chain_falls_back_to_wei(self) -> None:
        """Test unknown chain falls back to 'Wei' default."""
...
class TestMakeChainLedgerApi:
    """Tests for make_chain_ledger_api."""

    def test_solana_raises_not_implemented(self) -> None:
        """Test Solana chain raises NotImplementedError."""
        with pytest.raises(NotImplementedError, match="Solana"):
            make_chain_ledger_api(Chain.SOLANA)

    @pytest.mark.parametrize(
        "chain", [Chain.BASE, Chain.MODE, Chain.OPTIMISM, Chain.ROBINHOOD]
    )
    def test_fallback_max_fee_per_gas_capped_at_5_gwei(self, chain: Chain) -> None:
        """Test cheap ETH L2s set fallback maxFeePerGas to 5 GWEI."""
        captured: dict = {}
    ...
    def test_polygon_sets_max_gas_fast_and_max_fee_per_gas(self) -> None:
        """Test Polygon sets max_gas_fast and fallback maxFeePerGas."""
        captured: dict = {}
...
class TestUpdateTxWithGasPricing:
    """Tests for update_tx_with_gas_pricing."""
    ...
    def test_raises_when_gas_pricing_unavailable(self) -> None:
        """Test RuntimeError raised when gas pricing returns None."""
        mock_api = MagicMock()
    ...
    def test_applies_eip1559_gas_pricing(self) -> None:
        """Test EIP-1559 pricing sets maxFeePerGas and maxPriorityFeePerGas."""
        mock_api = MagicMock()
    ...
    def test_applies_legacy_gas_price(self) -> None:
        """Test legacy pricing sets gasPrice."""
        mock_api = MagicMock()
    ...
    def test_raises_on_invalid_pricing_format(self) -> None:
        """Test RuntimeError raised when pricing format is unrecognised."""
        mock_api = MagicMock()
...
class TestUpdateTxWithGasEstimate:
    """Tests for update_tx_with_gas_estimate."""

    def test_first_address_succeeds(self) -> None:
        """Test gas estimated successfully on first address."""
    ...
    def test_fallback_address_used_when_first_fails(self) -> None:
        """Test fallback address is tried when primary address fails."""
    ...
    def test_all_addresses_fail_restores_original_gas(
        ...
        """Test original gas restored and warning printed when all addresses fail."""
        mock_api = MagicMock()
    ...
    def test_custom_multiplier_is_applied(self) -> None:
        """Test custom gas_estimate_multiplier is applied to final gas."""
